module.exports = {
  config: {
    ...
    longDescription: {
      en: "يضيف هذا الأمر المستخدم إلى مجموعة مجمتمع_دعم_البوت التي يريدها المستخدم فقط.",
    },
    category: "المجموعة",
    guide: {
  ...
  onStart: async function ({ api, args, message, event }) {
    const supportGroupId = "7687785661327701"; // ID of the support group
    ...
    if (participantIDs.includes(userID)) {
      ...
      api.sendMessage(
        "╔═════════ஜ۩۞۩ஜ═════════╗\n\n ⚠️ |أنت بالفعل في مجموعة البوت إذا لم تجده، يرجى التحقق من طلبات الرسائل أو قائمة الإنتظار.\n\n╚═════════ஜ۩۞۩ஜ═════════╝",
        threadID
      );
    } else {
...
api.setMessageReaction("🚫", event.messageID, (err) => {}, true);
      ...
      api.addUserToGroup(userID, supportGroupId, (err) => {
        if (err) {
          console.error("╔═════════ஜ۩۞۩ஜ═════════╗\n\n ❌ |فشلت إضافة مستخدم إلى مجموعة  مجتمع_دعم_البوت :\n\n╚═════════ஜ۩۞۩ஜ═════════╝", err);
