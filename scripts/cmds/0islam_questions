...
module.exports = {
  ...
  onStart: async function ({ message, event, commandName }) {
    let questions = [];
    try {
      questions = JSON.parse(fs.readFileSync('hazoura.json'));
    } catch (error) {
      return message.reply("❌ | حدث خطأ في قراءة ملف الأسئلة. يرجى التحقق من الملف.");
    }

    if (!Array.isArray(questions) || questions.length === 0) {
      return message.reply("❌ | لم يتم العثور على أسئلة في ملف الأسئلة.");
    }
